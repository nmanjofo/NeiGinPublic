...
void ImGui::PopStyleVar(int count)
{
    ...
    while (count > 0)
    {
        // We avoid a generic memcpy(data, &backup.Backup.., GDataTypeSize[info->NodeType] * info->Count), the overhead in Debug is not worth it.
...
void ImGui::LoadIniSettingsFromMemory(const char* ini_data, size_t ini_size)
{
    ...
    for (char* line = buf; line < buf_end; line = line_end + 1)
    {
        ...
        {
            // Parse "[NodeType][Name]". Note that 'Name' can itself contains [] characters, which is acceptable with the current format and parsing code.
            line_end[-1] = 0;
